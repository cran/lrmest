lte3<-function (formula, k, d, press = FALSE, data = NULL, na.action, 
    ...) 
{
    k <- as.matrix(k)
    d <- as.matrix(d)
    ...
    ltes3 <- function(formula, k1, d1, data = NULL, na.action, 
        ...
        ev <- (t(y) %*% y - t(bb) %*% t(x) %*% y)/(NROW(x) - 
            NCOL(x))
        ev <- diag(ev)
        dbd <- ev * (solve(s + I) + d1 * solve(s + I) %*% solve(s + 
            k1 * I)) %*% s %*% (solve(s + I) + d1 * solve(s + 
            k1 * I) %*% solve(s + I))
        ...
        bibet <- ((solve(s + I) + d1 * solve(s + I) %*% solve(s + 
            k1 * I)) %*% s - I) %*% bb
        ...
        for (i in 1:nrow(x)) {
            subsum <- 0
            bb <- c(x[i, ] %*% t(x[i, ]))
            z <- (solve(s + I - bb) + d1 * solve(s + I - bb) %*% 
                solve(s + k1 * I - bb)) %*% (t(x) %*% y - x[i, 
                ] * y[i])
        ...
        anw <- list(`*****Type (3) Liu Estimator*****` = ans, 
            ...
            for (i in 1:nrow(d)) {
                ltem3 <- function(formula, k, d, data, na.action, 
                  ...
                  ev <- (t(y) %*% y - t(bb) %*% t(x) %*% y)/(NROW(x) - 
                    NCOL(x))
                  ev <- diag(ev)
                  dbd <- ev * (solve(s + I) + d * solve(s + I) %*% 
                    solve(s + k * I)) %*% s %*% (solve(s + I) + 
                    d * solve(s + k * I) %*% solve(s + I))
                  bibet <- ((solve(s + I) + d * solve(s + I) %*% 
                    solve(s + k * I)) %*% s - I) %*% bb
                  bibets <- bibet %*% t(bibet)
                  mse <- dbd + bibets
                  mse1 <- sum(diag(mse))
    ...
    plotprlt3 <- function(formula, k, d, data = NULL, na.action, 
        ...
        for (j in 1:nrow(k)) {
            for (i in 1:nrow(d)) {
                pre11 <- function(formula, k, d, data, na.action, 
                  ...
                  for (i in 1:nrow(x)) {
                    ...
                    z <- (solve(s + I - bb) + d * solve(s + I - 
                      ...
                      y - x[i, ] * y[i])
                    for (m in 1:ncol(x)) subsum <- subsum + (x[i, 
                      m] %*% z[m])
                    pr <- pr + (y[i] - subsum)^2
                  }
                  pre <- t(pr)
                ...
                arr[j * i] <- pre11(formula, k[j], d[i], data, 
                  na.action)
