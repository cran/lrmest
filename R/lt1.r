lte1<-function (formula, k, d, press = FALSE, data = NULL, na.action, 
    ...) 
{
    k <- as.matrix(k)
    d <- as.matrix(d)
    ...
    ltes1 <- function(formula, k1, d1, data = NULL, na.action, 
        ...
        dbd <- ev * ((solve(s + k1 * I) - d1 * solve(s + k1 * 
            I) %*% solve(s)) %*% s %*% (solve(s + k1 * I) - d1 * 
            solve(s) %*% solve(s + k1 * I)))
        ...
        bibet <- (solve(s + k1 * I) %*% s - d1 * solve(s + k1 * 
            I) - I) %*% bb
        ...
        anw <- list(`*****Type (1) Liu Estimator*****` = ans, 
    ...
    plotprlt1 <- function(formula, k, d, data = NULL, na.action, 
        ...
        for (j in 1:nrow(k)) {
            for (i in 1:nrow(d)) {
                ...
                arr[j * i] <- pre(formula, k[j], d[i], data, 
                  na.action)
