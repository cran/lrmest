rrre<-function (formula, r, R, dpn, delt, k, data = NULL, na.action, 
    ...) 
{
    k <- as.matrix(k)
    k1 <- k[1L]
    rrres <- function(formula, r, R, dpn, delt, k1, data = NULL, 
        na.action, ...) {
        ...
        dbd <- ev * wk %*% (solve(s) - solve(s) %*% t(RR) %*% 
        ...
        dbd <- ev * wk %*% (solve(s) - solve(s) %*% t(RR) %*% 
            (RR %*% solve(s) %*% t(RR)) %*% RR %*% solve(s)) %*% 
            t(wk)
        bibet <- wk %*% solve(s) %*% t(RR) %*% solve(RR %*% solve(s) %*% 
            t(RR)) %*% del - k1 * solve(s + k1 * I) %*% bb
        ...
        anw <- list(`*****Restricted Ridge Regression Estimator*****` = ans, 
            `*****Mean square error value*****` = mse1)
        return(anw)
    }
    npt <- rrres(formula, r, R, dpn, delt, k1, data, na.action)
    plotrrr <- function(formula, r, R, dpn, delt, k, data = NULL, 
        na.action, ...) {
        j <- 0
        arr <- 0
        for (j in 1:nrow(k)) {
            rrrem <- function(formula, r, R, dpn, delt, k, data, 
                na.action, ...) {
                cal <- match.call(expand.dots = FALSE)
                ...
                dbd <- ev * wk %*% (solve(s) - solve(s) %*% t(RR) %*% 
                  (RR %*% solve(s) %*% t(RR)) %*% RR %*% solve(s)) %*% 
                  t(wk)
                bibet <- wk %*% solve(s) %*% t(RR) %*% solve(RR %*% 
                  solve(s) %*% t(RR)) %*% del - k * solve(s + 
                  k * I) %*% bb
                bibets <- bibet %*% t(bibet)
                mse <- dbd + bibets
                mse1 <- sum(diag(mse))
            ...
            arr[j] <- rrrem(formula, r, R, dpn, delt, k[j], data, 
                na.action)
        }
        MSE <- arr
        Parameter <- k
