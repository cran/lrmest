lte2<-function (formula, k, d, press = FALSE, data = NULL, na.action, 
    ...) 
{
    k <- as.matrix(k)
    d <- as.matrix(d)
    ...
    ltes2 <- function(formula, k1, d1, data = NULL, na.action, 
        ...
        ev <- (t(y) %*% y - t(bb) %*% t(x) %*% y)/(NROW(x) - 
            NCOL(x))
        ev <- diag(ev)
        dbd <- ev * (solve(s + k1 * I) - d1 * solve(s + k1 * 
            I) %*% solve(s + k1 * I)) %*% s %*% (solve(s + k1 * 
            I) - d1 * solve(s + k1 * I) %*% solve(s + k1 * I))
        ...
        bibet <- ((solve(s + k1 * I) - d1 * solve(s + k1 * I) %*% 
            solve(s + k1 * I)) %*% s - I) %*% bb
        ...
        anw <- list(`*****Type (2) Liu Estimator*****` = ans, 
            ...
            for (i in 1:nrow(d)) {
                ltem2 <- function(formula, k, d, data, na.action, 
                  ...
                  ev <- (t(y) %*% y - t(bb) %*% t(x) %*% y)/(NROW(x) - 
                    NCOL(x))
                  ev <- diag(ev)
                  dbd <- ev * (solve(s + k * I) - d * solve(s + 
                    k * I) %*% solve(s + k * I)) %*% s %*% (solve(s + 
                    k * I) - d * solve(s + k * I) %*% solve(s + 
                    k * I))
                  bibet <- ((solve(s + k * I) - d * solve(s + 
                    k * I) %*% solve(s + k * I)) %*% s - I) %*% 
                    bb
                  bibets <- bibet %*% t(bibet)
                  mse <- dbd + bibets
    ...
    plotprlt2 <- function(formula, k, d, data = NULL, na.action, 
        ...
        for (j in 1:nrow(k)) {
            for (i in 1:nrow(d)) {
                ...
                arr[j * i] <- pre1(formula, k[j], d[i], data, 
                  na.action)
