alte2<-function (formula, k, d, aa, press = FALSE, data = NULL, na.action, 
    ...) 
{
    k <- as.matrix(k)
    d <- as.matrix(d)
    ...
    altes2 <- function(formula, k1, d1, aa, data = NULL, na.action, 
        ...
        dbd <- ev * A %*% (solve(s + k1 * I) - d1 * solve(s + 
            k1 * I) %*% solve(s + k1 * I)) %*% s %*% (solve(s + 
            k1 * I) - d1 * solve(s + k1 * I) %*% solve(s + k1 * 
            I)) %*% t(A)
        ...
        bibet <- A %*% ((solve(s + k1 * I) - d1 * solve(s + k1 * 
            I) %*% solve(s + k1 * I)) %*% s - I) %*% bb
        ...
        for (i in 1:nrow(x)) {
            subsum <- 0
            bb1 <- c(x[i, ] %*% t(x[i, ]))
            z <- (solve(s + k1 * I - bb1) - d1 * solve(s + k1 * 
                I - bb1) %*% solve(s + k1 * I - bb1)) %*% (t(x) %*% 
                y - x[i, ] * y[i])
        ...
        anw <- list(`*****Type (2) Adjusted Liu Estimator*****` = ans, 
    ...
    plotpralt2 <- function(formula, k, d, aa, data = NULL, na.action, 
        ...
        for (j in 1:nrow(k)) {
            for (i in 1:nrow(d)) {
                ...
                arr[j * i] <- apre1(formula, k[j], d[i], aa, 
                  data, na.action)
