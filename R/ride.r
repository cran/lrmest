rid<-function (formula, k, data = NULL, na.action, ...) 
{
    k <- as.matrix(k)
    k1 <- k[1L]
    rides <- function(formula, k1, data = NULL, na.action) {
        ...
        ev <- (t(y) %*% y - t(bb) %*% t(x) %*% y)/(NROW(x) - 
            NCOL(x))
        ev <- diag(ev)
        dbd <- ev * solve(s + k1 * I) %*% s %*% solve(s + k1 * 
            I)
        ...
        anw <- list(`*****Ordinary Ridge Regression Estimators*****` = ans, 
