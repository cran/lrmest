rls<-function (formula, r, R, delt, data, na.action, ...) 
{
    ...
    ev <- diag(ev)
    dbd <- ev * solve(s) - ev * solve(s) %*% t(RR) %*% solve(RR %*% 
        solve(s) %*% t(RR)) %*% RR %*% solve(s)
    Standard_error <- sqrt(diag(abs(dbd)))
    dbd <- ev * solve(s) - ev * solve(s) %*% t(RR) %*% solve(RR %*% 
        solve(s) %*% t(RR)) %*% RR %*% solve(s)
    bibet <- solve(s) %*% t(RR) %*% solve(RR %*% solve(s) %*% 
        t(RR)) %*% del
    ...
    anw <- list(`*****Restricted Least Square Estimator*****` = ans, 
